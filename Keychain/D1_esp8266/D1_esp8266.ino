...
int servoStep = 1;
bool flower = false;
unsigned long vibrateStart = 0; // Timestamp of vibration motor
bool vibrate = false;
...
void loop() {
  ...
  if (shake > 1500) {  //Define the shake
    ...
    vibrate = true;    // Start vibrating if a shake is detected
    vibrateStart = millis();
    digitalWrite(VIBRATION_PIN, HIGH);
  }

if (millis() - vibrateStart >= 5000) { // non-blocking timing
    digitalWrite(VIBRATION_PIN, LOW);  // Vibrate for 5 seconds
    vibrate = false;
}
